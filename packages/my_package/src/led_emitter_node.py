...
class LEDEmitterNode(DTROS):
    ...
    def __init__(self, node_name):
        ...
        # Create default LED protocol for simulation/testing
        self._LED_protocol = {
            "colors": {
                "red": [255, 0, 0],
                "green": [0, 255, 0], 
        ...
        self._LED_scale = 0.8
        self._channel_order = {"duckiebot": "RGB"}
        ...
        self.led_switch = True
        self.frequency = 0.0
        self.is_on = False
        
        # Initialize the timer (will be updated when pattern changes)
        ...
        self.log("LED Emitter Node Initialized.")
    ...
    def changePattern(self, pattern_name):
        ...
        """
        if pattern_name:
            # No need to change if we already have the right pattern, unless it is custom
            if self.current_pattern_name == pattern_name and pattern_name != "custom":
                ...
                self.log(str(list(self._LED_protocol["signals"].keys())), type="err")
                return

            # Extract the color from the protocol config
            ...
            # If static behavior, update LEDs
            if self.frequency == 0:
                self.updateLEDs()
    ...
    def changeFrequency(self):
        ...
        if self.frequency == 0:
            # No timer needed for static patterns
            pass
        else:
            try:
                # Convert to hz
    ...
    def on_shutdown(self):
        ...
        self.log("Shutting down. Turning LEDs off.")
...
if __name__ == "__main__":
    ...
    led_emitter_node = LEDEmitterNode(node_name="led_emitter_node")
    # Setup proper shutdown behavior
    rospy.on_shutdown(led_emitter_node.on_shutdown)
